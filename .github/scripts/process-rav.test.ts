...
import { getDate, extractImageUrl, getUniqueFilename, updateHtml } from './process-rav';
...
test('getDate: extracts date embedded in other text', () => {
  expect(getDate('spotted this today 20260315 near the shops')).toBe('20260315');
});
...
test('getUniqueFilename: appends _2 when base exists', () => {
  expect(getUniqueFilename('20260320', f => f === '20260320.jpg')).toBe('20260320_2.jpg');
});
...
test('updateHtml: updates og:image to new filename', () => {
  ...
  expect(result).toContain('content="https://rav4.cool/assets/ravs/20260320.jpg"');
  expect(result).not.toContain('content="https://rav4.cool/assets/ravs/20260318.jpg"');
});
